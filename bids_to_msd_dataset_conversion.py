...
if not os.path.exists(os.path.join(root, 'participants.tsv')):
    raise FileNotFoundError("participants.tsv file not found in the dataset directory. Cannot proceed with subject selection.")
        ...
        for file in files:
            ...
            if sessionID == '':
                ...
                if args.group_by_contrasts:
                    ...
                    assert args.common_label_contrast != '', "Please specify the common label contrast using the --common_label_contrast argument. " \
                                                                "This will be used to pair all the contrasts with the same label file."
                    ...
                    # Similar to nnUNet's naming convention, each contrast will be stored as image_0000, image_0001, etc. Since the label is common for 
                    # all the contrasts, it will be stored as label_0000
                ...
                if args.group_by_sessions:
                    ...
                    if args.group_by_contrasts:
                        ...
                        print("Grouping by both sessions and contrasts is not currently supported. Grouping by sessions only supports a single contrast")
                        exit() 
...
# get the number of training, validation and test samples depend on the depending on the grouping/sessions/contrasts chosen
